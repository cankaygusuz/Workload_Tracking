...
<!--     <link rel="stylesheet" type="text/css" href="css/semantic2.min.css"/>
		...
		<!--	<div class="ui search">
            ...
            <div class="ui modal modal_edit">
				...
				<form action ="" method="POST">
					...
					<label class="labels">DEPT : </label><input type="text" name="department" id="dis_dep"  /><br/>
					...
					<label class="labels"></label><input class="updt" id="updt" type="button" name="update" value="UPDATE DATA" onclick="editselectdrp()" style="margin-left: 30px!important;" />
				</form>
 ...
 ?>*/
				...
				//print_r($_SESSION['tb_text']);
				//	
				echo '<select name="unitid" onchange="" id="select_feature">'; //$("#select_feature").val();
				//while ($row = mysqli_fetch_assoc($qres)) {
					echo "<option selected disabled>Choose column</option>";
			//	}
						while ($row = mysqli_fetch_assoc($qres)) {
					//	echo "<option value='" . $row['col_of_fac'] . "'>" . $row['col_of_fac'] . "</option>";
						echo "<option value='"  .$row['depname']. "'>" . $row['depname'] . "</option>";
						}
					echo "</select>";
				//	echo '<input type="text" id="fname" name="fname" value="'.$_SESSION['tb_text'].'">';
					
						//echo '<tr>';
						//echo '</tr>';
					//	echo '<input type="text" id="fname" name="fname" value="'.$deg.'">';*/
//ob_flush();
			?>
		...
		?>
         ...
         <!--       <table class="ui celled striped table"> -->
			...
			<?php
			$sum2 = $sum / $j;
			echo '<thead style="background-color: darkcyan;">';
